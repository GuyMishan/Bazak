...
const CandidatePreferenceFilter = (props) => {
    ...
    const calculateUnitsAccordingToData = async (data) => {
        let tempunits = [];
        for (let i = 0; i < data.length; i++) {
            if (!isDuplicate(tempunits, data[i].candidate.user.job.unit)) {
                tempunits.push(data[i].candidate.user.job.unit)
            }
        }
        setUnits(tempunits);
    }
                            ...
                            {migzars ? migzars.map((migzar, index) => {
                                {
                                    return (
                                        <Row>
                                            {props.candidatepreferencefilter.migzarfilter && props.candidatepreferencefilter.migzarfilter.indexOf(migzar) != -1 ?
                            ...
                            {units ? units.map((unit, index) => {
                                {
                                    return (
                                        <Row>
                                            {props.candidatepreferencefilter.unitfilter && props.candidatepreferencefilter.unitfilter.indexOf(unit._id) != -1 ?
                            ...
                            {movements ? movements.map((movement, index) => {
                                {
                                    return (
                                        <Row>
                                            {props.candidatepreferencefilter.movementfilter && props.candidatepreferencefilter.movementfilter.indexOf(movement._id) != -1 ?
...
export default withRouter(CandidatePreferenceFilter);;