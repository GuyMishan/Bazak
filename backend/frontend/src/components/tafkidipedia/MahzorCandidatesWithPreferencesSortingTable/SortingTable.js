...
const SortingTable = ({ match }) => {
  ...
  function init() {
    getMahzorCabdidatesWithPreferences();
  }

  const getMahzorCabdidatesWithPreferences = async () => {
    let tempcandidateswithpreferences = [];
    ...
    let tempactivecandidates = [];
    for (let i = 0; i < tempdata.length; i++) {
      if (tempdata[i].mahzor._id == match.params.mahzorid) {
        tempactivecandidates.push(tempdata[i].candidate)
      }
    }

    //saves only candiidates with a preference
    for (let k = 0; k < tempallcandidates.length; k++) {
      let flag = false;
      for (let l = 0; l < tempactivecandidates.length; l++)
        if (tempallcandidates[k]._id == tempactivecandidates[l]._id) {
          flag = true;
        }
      if (flag == true) {
        tempcandidateswithpreferences.push(tempallcandidates[k]);
      }
    }
    setData(tempcandidateswithpreferences)
    setOriginalData(tempcandidateswithpreferences)
  }
    ...
    <>
      ...
      <div style={{ float: 'right' }}>
        <ReactHTMLTableToExcel
          ...
          <Link style={{color: 'inherit', textDecoration: 'inherit', fontWeight: 'inherit' }} to={`/mahzorcandidatespreferecespage/${match.params.mahzorid}`}> <button className="btn-new"style={{ padding: "0.5rem" }}>לחץ לפרטים</button></Link>
      </div>
                  ...
                  <tr {...row.getRowProps()}>
                    {
                      row.cells.map(cell => {
                        if (cell.column.id != "user.name") {
                          return <td style={{width:`${100/4}%`,minWidth:'125px'}} {...cell.getCellProps()}>{cell.render('Cell')}</td>
                        }
                        else {
                          return <td style={{width:`${100/4}%`,minWidth:'125px'}}><Link style={{color: 'inherit', textDecoration: 'inherit', fontWeight: 'inherit' }} to={`/profilepage/${row.original.user._id}`}>{cell.value}{" "}{row.original.user.lastname}</Link></td>
                        }
                      })
                    }
                  </tr>
